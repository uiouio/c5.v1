...
{
    public class CardAssignCommand : AbstractInstructionCommand
    {
        public CardAssignCommand(ChunkedProducerConsumer<IInstruction> instructionProducer)
        ...
        protected override Rpc.Serial.IInstruction BuildInstruction(Rpc.Net.Message.DuplexMessage commandMessage)
        {
            ...
            using (var scope = ObjectHost.Host.BeginLifetimeScope())
            {
                ...
                {
                    instrSet.AddInstruction(scope.Resolve<CardAssignInstruction>(
                        new NamedParameter("command", this),
                        new NamedParameter("parameter", card)));
                    if (card.Profile != null)
                    {
                        instrSet.AddInstruction(scope.Resolve<StudentProfileSyncInstruction>(
                            new NamedParameter("command", this),
                            new NamedParameter("parameter", card.Profile)));
                    }
                }
            }
            return instrSet;
